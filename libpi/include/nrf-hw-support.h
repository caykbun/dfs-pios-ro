...
// toggle RX
static inline void nrf_enable_rx(nrf_t* n, uint32_t rx) {
    assert(rx >= 0 && rx < 6);
    nrf_put8(n, NRF_EN_RXADDR, ((1 << rx) | nrf_get8(n, NRF_EN_RXADDR)));
}
// disable a single rx 
static inline void nrf_disable_rx(nrf_t* n, uint32_t rx) {
    // CAUTION: the RX FIFO might contain data from this rx after disabling this rx, need to verify the other side's address first in that case
    assert(rx >= 0 && rx < 6);
    nrf_put8(n, NRF_EN_RXADDR, (~(1 << rx) & nrf_get8(n, NRF_EN_RXADDR)));
}
