...
void sw_uart_put8(sw_uart_t *uart, unsigned char c) {
    ...
    for (unsigned i = 0; i < 8; i++) {
        write_cyc_until(uart->tx, (c >> i) & 0b1, start + ((uart->cycle_per_bit) * (i+1)), (uart->cycle_per_bit));
    }
    write_cyc_until(uart->tx, 1, start + ((uart->cycle_per_bit) * 9), uart->cycle_per_bit);
}
...
sw_uart_t sw_uart_mk_helper(unsigned tx, unsigned rx,
    ...
    gpio_set_on(tx);
    dev_barrier();
    delay_ncycles(cycle_cnt_read(), cyc_per_bit);
