...
int _wrap_spi_n_transfer(spi_t s, uint8_t rx[], const uint8_t tx[], unsigned nbytes) {
    // if not disable int and enable, sometimes PANIC:new-spi.c:end_transfer:290:selected wrong chip: cs=1, chip=0
    // the save and load is not correct
    // cpsr_int_disable();
    int res = spi_n_transfer(s, rx, tx, nbytes);
    // cpsr_int_enable();
    return res;
}
...
uint8_t nrf_get8(const nrf_t *n, uint8_t reg) {
    ...
    _wrap_spi_n_transfer(n->spi, rx,tx,2);
...
uint8_t nrf_getn(nrf_t *nic, uint8_t cmd, void *bytes, uint32_t nbytes) {
    ...
    _wrap_spi_n_transfer(nic->spi, rx,tx,nbytes+1);
...
uint8_t nrf_tx_flush(const nrf_t *n) {
    ...
    _wrap_spi_n_transfer(n->spi, &res, &cmd, 1);
    return res;     // status.
}
